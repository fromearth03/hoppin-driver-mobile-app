...
void main() {
  test('LIVE-01 — going online sends a heartbeat IMMEDIATELY', () {
    FakeAsync().run((async) {
      ...
      expect(h.repo.pings, isEmpty,
          reason: 'an OFFLINE driver is not in the pool and must not ping');

      h.goOnline();
      async.elapse(const Duration(milliseconds: 100));

      expect(h.repo.pings, hasLength(1),
          reason: 'the ping fires IMMEDIATELY, not on the first 20s tick. '
              'goOnline() has already returned 200, so the driver is in the '
              'pool RIGHT NOW — a 20-second gap is 20 seconds in which dispatch '
              'can match them from no fix at all');
      expect(h.repo.pings.single.lat, _Loc.defaultFix.lat,
          reason: 'the ping carries the DEVICE fix, not a constant');
      expect(h.repo.pings.single.lng, _Loc.defaultFix.lng,
          reason: 'the ping carries the DEVICE fix, not a constant');
      expect(h.state.phase, PresencePhase.live,
  ...
  test('LIVE-02 — three minutes online yields MANY heartbeats', () {
    FakeAsync().run((async) {
      ...
      // The server drops a driver after 5 MINUTES with no ping. At the 20s
      // cadence a 3-minute shift owes ~9 pings; anything at or below 2 would
      // mean presence lapses and the driver silently leaves the pool.
      expect(h.repo.pings.length, greaterThan(2),
          reason: 'dispatchability must survive the server 5-minute presence '
              'window — 3 minutes of shift must produce far more than 2 pings');
      expect(h.repo.pings.length, greaterThanOrEqualTo(8),
          reason: 'the 20s cadence gives 15 misses of headroom before presence '
              'lapses; ~9 pings in 3 minutes is the shape of that guarantee');
      expect(h.state.pingCount, h.repo.pings.length,
  ...
  test('going offline STOPS the heartbeat', () {
    FakeAsync().run((async) {
      ...
      expect(h.repo.pings, hasLength(whileOnline),
          reason: 'an offline driver is NOT in the dispatch pool and must not '
              'keep telling the server where they are — that is both a lie to '
              'dispatch and a battery drain');
      expect(h.state.phase, PresencePhase.idle,
  ...
  test('NO FIX → ZERO pings, and the driver is TOLD (never 0,0)', () {
    FakeAsync().run((async) {
      ...
      expect(h.repo.pings, isEmpty,
          reason: '🔴 NEVER POST A FABRICATED COORDINATE. The server ACCEPTS an '
              ...
              'NO PING');
      expect(h.state.phase, PresencePhase.noFix,
          reason: 'the state says the driver is unreachable');
      expect(h.state.locationUnavailable, isTrue,
          reason: 'this is what raises the #84 LocationUnavailableBanner on the '
              'dashboard — the app must never show a confident ONLINE state '
              'over a driver nobody can reach');
      expect(h.state.pingCount, 0,
  ...
  test('a failing WIRE degrades but keeps retrying (presence has 5 min)', () {
    FakeAsync().run((async) {
      ...
      expect(h.state.phase, PresencePhase.degraded,
          reason: 'the fix is good; the wire is not — that is a different '
              'state from having no fix, and it is not the #84 rung');
      expect(h.state.locationUnavailable, isFalse,
          reason: 'a network blip is NOT an OS permission problem — raising the '
              '"turn on location" banner here would send the driver to a '
              'settings page that fixes nothing');
      expect(h.repo.attempts, greaterThan(1),
          reason: 'the timer keeps trying — presence only lapses if nothing '
  ...
  group('the foreground service (the shift survives a locked screen)', () {
    test('going online STARTS the foreground service', () {
      FakeAsync().run((async) {
        final h = _Harness();
        async.flushMicrotasks();
        expect(h.shift.starts, 0,
            reason: 'an offline driver owes the OS no persistent notification '
                'and no wake lock');

        h.goOnline();
        async.elapse(const Duration(milliseconds: 100));

        expect(h.shift.starts, 1,
            reason: '🔴 THE WHOLE POINT OF PHASE 2. Without the foreground '
                'service Android is free to freeze the isolate the moment the '
                'app leaves the screen — the 20s timer stops, the heartbeat '
                'starves, and the admin drops the driver from the pool 5 '
                'minutes later. The service is what keeps the isolate ALIVE');
        expect(h.state.shiftServiceRunning, isTrue,
            reason: 'the state records what the OS actually did, not what we '
                'asked it to do');
        expect(h.state.backgroundProtected, isTrue,
            reason: 'full coverage + a running service = the shift genuinely '
    ...
    test('going offline STOPS the foreground service', () {
      FakeAsync().run((async) {
        ...
        expect(h.shift.stops, greaterThanOrEqualTo(1),
            reason: 'a persistent "On shift" notification over a driver who has '
                'clocked off is its own small lie — and the wake lock is not '
                'free');
        expect(h.state.shiftServiceRunning, isFalse,
            reason: 'the state follows the world');
        expect(h.state.backgroundProtected, isFalse,
            reason: 'an offline driver is not protected because there is '
                'nothing to protect');
    ...
    test(
        '🔴 the service REFUSING to start is DISCLOSED, never swallowed '
      ...
      FakeAsync().run((async) {
        ...
        expect(h.repo.pings, isNotEmpty,
            reason: 'the driver is still DISPATCHABLE — the heartbeat runs in '
        ...
        expect(h.state.backgroundProtected, isFalse,
            reason: '🔴 THE PERMISSION IS NOT THE PROTECTION. Holding '
        ...
        expect(h.state.foregroundOnly, isTrue,
            reason: 'the CONSEQUENCE is identical to a foreground-only grant '
  ...
  group('🔴 the two-stage permission flow (seam #85)', () {
    test(
        'a FOREGROUND-ONLY driver is STILL DISPATCHABLE — and is told exactly '
        'what they lose', () {
      FakeAsync().run((async) {
        final h = _Harness(
        ...
        expect(h.repo.pings.length, greaterThan(2),
            reason: '🔴 A FOREGROUND-ONLY DRIVER IS NOT A BROKEN DRIVER. They '
        ...
        expect(h.state.locationUnavailable, isFalse,
            reason: 'this is NOT the #84 no-fix state. Raising the "turn on '
        ...
        expect(h.state.coverage, DriverLocationCoverage.foregroundOnly,
            reason: 'coverage is read from the OS, not assumed from the fact '
                'that a fix arrived — a fix arrives in BOTH worlds while the '
                'app is on screen, which is exactly what makes this lie so easy '
                'to ship');
        expect(h.state.backgroundProtected, isFalse,
            reason: '🔴 THE ASSERTION THIS WHOLE PHASE EXISTS FOR. The app must '
                ...
                'payment card');
        expect(h.state.foregroundOnly, isTrue,
            reason: 'this is what mounts BackgroundLocationLimitedNotice on the '
    ...
    test('stage two: the driver taps the rung, the OS says YES', () {
      FakeAsync().run((async) {
        ...
        expect(h.state.coverage, DriverLocationCoverage.full,
            reason: 'the OS granted "Allow all the time" and we re-read it FROM '
                'THE OS — never from our own request\'s return value, because '
                'the driver may still be standing in the Settings app');
        expect(h.state.backgroundProtected, isTrue,
            reason: 'full grant + running service: the shift now genuinely '
                'survives a locked screen, and the badge may finally be '
                'confident');
        expect(h.state.foregroundOnly, isFalse,
            reason: 'the #85 rung disappears — because the condition it '
    ...
    test(
        'stage two: the OS says NO (Android 11+ shows no dialog) → the driver '
      ...
      FakeAsync().run((async) {
        ...
        expect(h.loc.settingsOpened, 1,
            reason: '🔴 A DISCLOSURE THAT STRANDS THE DRIVER IS ONLY HALF '
                ...
                'is a shrug');
        expect(h.state.backgroundProtected, isFalse,
            reason: 'the driver has not granted it yet — they are still walking '
                'to Settings. We do not credit them for a permission they have '
                'not given');
        expect(h.state.foregroundOnly, isTrue,
            reason: 'the rung STAYS. It disappears when the OS says the grant '
    ...
    test(
        '🔴 coverage REVOKED mid-shift from Settings is caught on the next '
        'resume', () {
      FakeAsync().run((async) {
        final h = _Harness();
        h.goOnline();
        ...
        expect(h.state.backgroundProtected, isTrue,
            reason: 'sanity: nothing has told us yet — this is precisely the '
        ...
        expect(h.state.coverage, DriverLocationCoverage.foregroundOnly,
            reason: 'we ASKED THE OS AGAIN rather than trusting what we '
                'remembered. That is the only honest way to know');
        expect(h.state.backgroundProtected, isFalse,
            reason: 'the badge drops out of confidence the moment the truth '
                'does');
        expect(h.state.foregroundOnly, isTrue,
    ...
    test('an OFFLINE driver has nothing to refresh (no spurious OS calls)', () {
      FakeAsync().run((async) {
        ...
        expect(h.state.coverage, DriverLocationCoverage.none,
            reason: 'an offline driver is not covered because they are not '
...
class _Harness {
  ...
  void goOnline() => (container.read(dashboardInteractorProvider.notifier)
          as _StubDashboard)
      .set(DashboardPhase.online);

  void goOffline() => (container.read(dashboardInteractorProvider.notifier)
          as _StubDashboard)
...
class _RecordingDriverRepo implements DriverRepository {
  ...
  dynamic noSuchMethod(Invocation invocation) =>
      throw UnimplementedError('presence must not call ${invocation.memberName}');
}
